...
finesemilogx(0,0,'off','Approximate Transfer Function Frequency Response Bode Plot','\omega (rad/s)    .','Y(j\omega)',[-1 2],[-90 30],'on',[700 700],'Magnitude','r-');
b1 = semilogx(w,20*log10(abs(G)),'r-','DisplayName','Magnitude');
b2 = semilogx(w,angle(G)*180/pi,'r--','DisplayName','Phase');
...
y = 10*sin(t*0.1);
s2 = fineplot(t,y,'Voltage vs Time for \omega = 0.1','t (sec)','y(t) (V)',[0 80],[-80 80],'on',[400 400],'Input','k-');
...
for i = 1: length(w_exp)
    ...
    hold on
    grid on
    fineplot(t,y,strcat('Voltage vs Time for \omega=',num2str(w_exp(i))),'.   t (sec)','y(t)',[0 xboundary(i)],[-1*yboundary(i) yboundary(i)],'on',[2000 2000],'','k-');
    plot(waves(i),'b-')
end
%% Question 2 Graph
clf;
...
hold on
m2=finesemilogx(w,20*log10(abs(G)),'off','Magnitude vs Angular Frequency','\omega (rad/s)       .','|Y(j\omega)|',[-1 2],[0 40],'off',[600 1200],'Approximate Transfer Function','k-');
grid on
legend([m1 m2],'Location','southwest')
subplot(2,1,2)
p1 = semilogx(w_exp,phases*180/pi,'x','DisplayName','DC Motor Readings','MarkerSize',8,'LineWidth',2);
hold on
p2 = finesemilogx(w,angle(G)*180/pi,'off','Phase vs Angular Frequency','\omega (rad/s)','\angle Y(j\omega)',[-1 2],[30 -100],'off',[600 1200],'Approximate Transfer Function','k-');
...
m1=semilogx(w_exp,20*log10(abs(mags)),'x','DisplayName','DC Motor Readings','MarkerSize',8,'LineWidth',2);
hold on
m2=finesemilogx(w,20*log10(abs(G)),'off','Magnitude vs Angular Frequency','\omega (rad/s)','|Y(j\omega)|',[-1 2],[-15 40],'off',[600 1200],'Approximate Transfer Function','k-');
grid on 
legend([m1 m2],'Location','southwest')
subplot(2,1,2)
p1 = semilogx(w_exp,phases*180/pi,'x','DisplayName','DC Motor Readings','MarkerSize',8,'LineWidth',2);
hold on
p2 = finesemilogx(w,angle(G)*180/pi,'off','Phase vs Angular Frequency','\omega (rad/s)','\angle Y(j\omega)',[-1 2],[30 -150],'off',[600 1200],'Approximate Transfer Function','k-');
legend([p1 p2],'Location','southwest')
grid on
...
m3=finesemilogx(w,20*log10(abs(G)),'off','Magnitude vs Angular Frequency','\omega (rad/s)','|Y(j\omega)|',[-1 2],[-15 40],'off',[600 1200],'Approximate Transfer Function','k-');
grid on
...
p3 = finesemilogx(w,angle(G)*180/pi,'off','Phase vs Angular Frequency','\omega (rad/s)','\angle Y(j\omega)',[-1 2],[450 -450],'off',[600 1200],'Approximate Transfer Function','k-');
